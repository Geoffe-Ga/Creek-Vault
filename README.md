...
- **Right-to-be-forgotten.** `creek purge` deletes a fragment, source, date range, or the entire vault and scrubs every reference along the way: wiki-links by title across every `.md` file, word-boundary fragment-ID mentions in YAML provenance lists (`source_fragments: …` in drafts) and prose body text, plus the matching rows in `00-Creek-Meta/embeddings.parquet` (vault-purge deletes the cache file outright). When a deleted note is an intimate-tier save, the full body stub under `10-Liminal/Compost/intimate-stubs/` it points at is deleted too, so a scoped purge no longer leaves the intimate body on disk. The hash-chained purge audit log is the only artifact retained for compliance reconstruction.
