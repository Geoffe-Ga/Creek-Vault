...
> and `0.16.0` is now recorded in the [change log](#change-log) with the change
...
|---|---|---|
| `0.16.0` | 2026-09-11 | **The HTTP adapter gains consumer-scoped Journal withdrawal (#1799); the MCP tool surface does not move.** `DELETE /v1/journal-entries/{external_id}` is the idempotent destructive inverse of Journal upsert. It requires bearer identity, an explicit remote tier ceiling, and a negotiated `0.16` contract, but no request body or entry prose. A complete response means the caller's staged source, fragments, ledger/index/provenance references, and retrieval-facing derived state are absent; incomplete work is retryable and never reported as success. Foreign, unknown, already-withdrawn, and never-present identifiers are indistinguishable successes, so the route cannot be used as an ownership or existence oracle. The route, capability, and one closed response model widen the shared minor; `0.15` remains served without seeing or reaching them. |
| `0.15.0` | 2026-09-06 | **The HTTP adapter gains the `voice-drafts` capability (#1727); the MCP tool surface does not move.** `PUT`, `GET`, and `DELETE /v1/voice-drafts/{external_id}` create/update, recall, and retract one caller-keyed AI-authored draft. The stored fragment is fixed to `source.author=ai`, `source.author_slug=ai-as-user`, and `voice_weight=0.0` under `11-Other-Authors/ai-as-user`, so retrieval can see it and owner-voice generation cannot treat it as the user's words. The same id updates one deterministic slot; reads and mutations enforce the canonical ceiling; deletion supports retraction when the source becomes intimate without transmitting intimate prose. The three routes, capability, and five models widen the shared contract minor. `0.14` remains served, and is neither shown nor allowed to call the resource through the same since-minor gate. |
| `0.14.0` | 2026-09-05 | **The pipeline gains a durable asynchronous job surface (#1605).** `POST /v1/classifications` admits `method=llm` and `POST /v1/links` admits `method=embeddings`; both durably persist a consumer-bound job before answering `202 {status, job_id, state}`. `GET /v1/jobs/{job_id}` reports the closed `queued` / `running` / `succeeded` / `failed` lifecycle and, on success, the same counts-only classification or link model the synchronous route returns. Existing `rules`, `temporal`, `eddies`, and `threads` requests retain byte-identical `200` responses. Active records owned by a prior server instance become terminal `failed` after restart rather than looking live forever. Status responses carry no fragment id, path, title, prose, or tool error, and another authenticated consumer receives the same `unavailable` refusal as an unknown id. Admission permits one active detached pipeline job per authenticated consumer; another long-method request receives static `unavailable` until the worker finishes, bounding the otherwise-cheap `202` fan-out. The event loop alone mutates the admission sets, and status workers receive immutable snapshots. The detached classification worker calls the existing `classify_tool`, so cloud-enabled jobs still pass every intimate fragment through `ModelRouter._enforce_local_for_intimate`; an HTTP-level test observes that an intimate fragment reaches only the local provider. The new route, two enum members, `202` shape, and two wire models widen the HTTP contract, so the minor moves and `SUPPORTED_CONTRACT_MINORS` widens to keep `0.13` served. |
...
> **Every minor from `0.1.0` to `0.16.0` has a row above**, including the four
...
> which minors are served, and it currently spans `0.2`–`0.16`.
