...
def test_redaction_preserves_non_sensitive_text(noise: str) -> None:
    """Text containing no built-in pattern matches passes through unchanged.

    This guards against a regression where the redactor incorrectly
    treats arbitrary text as sensitive.
    """
    redactor = _make_redactor()
    redacted = redactor.redact_content(noise)
    # The redactor may legitimately substitute matches like email
    # addresses generated by hypothesis. Accept equality OR that the
    # output is shorter (some text was replaced) but never longer than
    # the input plus a sane upper bound.
    assert len(redacted) <= len(noise) + len(noise) * 3 + 64
