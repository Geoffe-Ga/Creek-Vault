...
| `creek.skills.refresh`  | none beyond `ceiling`                                       | Voice-skill tree regen. The ceiling is converted with `to_privacy_override` and threaded into `SkillTreeGenerator` as a **hard rank cutoff** (#971): an above-ceiling fragment is **omitted** from exemplar harvesting, never summarised, because a `[Personal-tier summary: <title>]` stub written into `## Exemplar Passages` would be a fabricated voice exemplar carrying the very title it claims to protect. Read that as a claim about **fragments** only: `Thread` and `Eddy` carry no `privacy_tier` field, so the generator's typed walk takes no override and **thread and eddy skills are not gated** — their titles, ids, descriptions and fragment counts are derived from their member fragments, so an eddy built from above-ceiling members still reaches `creek-skills/eddies/` and, slugified into the filename, reaches the returned `skill_paths` too (and moves `skill_count`). Gating that walk changes which files the tree emits at `ceiling=open`, so it is tracked separately in [#1284](https://github.com/Geoffe-Ga/Creek-Vault/issues/1284). The #971 gap was write-side — for the four fragment-derived categories the response returns only a count and fixed skill names — so the evidence is the bytes under the untiered `<vault>/creek-skills`. `unclassified` ranks with `personal` (#876), so a vault that has never been through `creek classify` yields a complete tree whose `## Exemplar Passages` sections all carry the "no qualifying exemplars" placeholder at `ceiling=open`; a broader ceiling recovers them. `intimate` needs more than a ceiling: it additionally requires the Python-API `allow_intimate` consent opt-in, which this tool never passes, so intimate exemplars are unreachable over MCP at **every** ceiling, `all` included. |
