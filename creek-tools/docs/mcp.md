...
| `creek.classify`        | `method` (`rules`\|`llm`), `force`, `retier`                | Rewrites in place; no new tier produced — any ceiling permitted. `retier` re-derives an already-recorded privacy tier and persists it only when the new verdict is **stricter** (#1106), so it can tighten a tier a write verb's caller declared wrongly and can never loosen one (#1570). |
