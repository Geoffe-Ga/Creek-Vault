...
What it *does* change is which fragments survive the top-`max_per_register` cut on the exemplar path, and that matters because a persisted register sample is the source fragment's file **copied byte for byte** into `07-Voice/Register-Samples/<register>/`, and those samples seed generated drafts. Concretely: with 20 borrowed (`reference`) fragments and one `self`-authored fragment competing for a cap of 20, `enabled: true` ranks the self-authored fragment above enough borrowed ones to keep it in the cohort; `enabled: false` flattens every authority to `1.0`, the self-authored fragment loses its ranking edge, and the corpus can become pure borrowed material. Turning the weighting off can therefore *increase* how much borrowed (or, with a permissive tier ceiling, private) prose is duplicated into the vault and fed to drafts — an operator-directed and permissible choice, but one worth knowing you are making.
