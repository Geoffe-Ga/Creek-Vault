...
The eleven capability routes — `PUT /v1/journal-entries/{external_id}`,
...
`0.10.0` (#1570) is the third such bump: `pipeline`, over `POST
/v1/classifications` and `POST /v1/links`. Additive in the same enforced sense,
with **no new error code and no new status**, so a `0.9` client meets nothing
new on any route it already calls. It closes the seeding gap — until it, `/v1`
could ingest and nothing else, so a vault seeded entirely over the network held
...
| Per-request timeout, **write routes** | **not enforced** | The caller waits for the real answer. `PUT /v1/journal-entries/{external_id}`, `POST /v1/uploads`, `POST /v1/connectors/drive/syncs`, `POST /v1/connectors/drive/authorizations`, `POST /v1/connectors/drive/authorizations/{state}`, `DELETE /v1/connectors/drive`, `POST /v1/classifications` and `POST /v1/links` mutate the vault, and a Python thread cannot be cancelled — only detached. Shedding one mid-mutation would hand the client a `503` about a write that in fact landed, which is worse than a late `200`. Budget accordingly: `POST /v1/links` on a cold embedding cache is minutes of work on a large vault. |
