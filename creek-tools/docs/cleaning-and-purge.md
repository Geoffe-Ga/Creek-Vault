...
| `link-through-a-wiped-alias` — a symlink that reaches an out-of-vault target *through* a second symlink the wipe destroys | unlinked |
| `link-traversing-a-wiped-directory` — a symlink whose relative target walks through a wiped directory before climbing back out to something that survives | unlinked |
| `link-named-in-another-case` — a symlink naming a content folder in a different case, which on a case-insensitive filesystem (macOS, by default) is the same directory | unlinked |
...
- **A symlink this purge breaks is unlinked, never followed — whatever it once pointed at.** `purge vault` wipes the ten content folders *before* it sweeps `00-Creek-Meta/`, so a link there that pointed into one of them is broken by the time the sweep meets it. That is true of a link to a fragment *file* and equally of a link to a *directory of fragments*: once the wipe has run, both are dangling, and a dangling link is destroyed regardless of what it used to resolve to. "Pointed into one of them" is judged over the whole **resolution chain**, not over the single path the link finally names — the three middle rows above are the shapes where those two differ. The residue that matters is not the body the link no longer reaches but the **target string** it carries, and this vault's paths are title-derived — `01-Fragments/Journal/2026-03-11 therapy session.md` is itself private text. Unlinking removes the alias and nothing else, so a link out of the vault is taken without its target being touched.
- **A symlink to a directory that outlives the purge is left alone**, matching the content wipe: the tree behind it is not this purge's to claim. That exemption is narrower than it looks. It holds for a target the purge does not destroy — an out-of-vault tree, or something under `00-Creek-Meta/` itself — *and* for a chain that reaches it without passing through a content folder. A link *into* a content folder is not a surviving-directory link; it is a link about to dangle, and it is unlinked. A link to a content folder **itself** is a surviving-directory link, because the wipe empties those ten folders without removing them.

`Meta artifacts removed` in a `--dry-run` preview is the number the apply run really performs, for every edge in the table above. The two runs meet different filesystems — a preview does not wipe the content folders, so a link into them is still resolvable at the moment the sweep classifies it — so the sweep asks whether this purge is *about to* destroy the link's target, not merely whether it already has. It answers by walking the link's **resolution chain** hop by hop and asking the *filesystem* whether any hop is reached through a content folder, comparing directories by identity (`samefile`) rather than by spelling. That is why a multi-hop alias, a relative target that traverses a wiped directory, and a spelling that differs only in case on a case-insensitive filesystem all classify the same way in both runs. It is **not an unconditional promise**: the walk gives up after 40 hops, so a symlink loop answers "not broken" — the loop is still unlinked, as dangling, because the kernel cannot resolve it either, but that verdict comes from the dangling rule rather than from this walk; it reads the filesystem as it stands at that moment rather than under a lock; and it errs toward *unlinking*, so a link whose chain merely passes through a wiped folder is taken even when the path it finally names survives — the `link-traversing-a-wiped-directory` row is exactly that case.

A loop used to be worse than imprecise: it made the preview *impossible*. `Path.resolve()` reports one as a `RuntimeError`, not the `OSError` the dry-run ledger's fallback was written to catch, so `creek purge vault --dry-run` over a vault holding `00-Creek-Meta/a -> b -> a` aborted where the apply run succeeded — the operator could not preview the erasure they were about to perform. The ledger now falls back to the literal spelling for that failure too, as it always intended to. Both runs err identically, which is the point: a preview that reports `0` before an apply that removes `1` is the same class of defect as #1485 itself, read in a mirror.
