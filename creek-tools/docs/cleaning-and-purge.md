...
3. **Scrubs every reference** — wiki-links **and markdown links** pointing at the deleted fragment(s) are removed from every other `.md` file in the vault, and every word-boundary mention of the deleted fragment ID is replaced with the `[purged]` placeholder across YAML frontmatter (e.g. `source_fragments: […]` in drafts and mining ideas) and body text. The walk covers all `.md` files in the vault, including `04-Praxis/`, `05-Wavelength/`, `06-Frequencies/`, `07-Voice/Drafts/`, `08-Decisions/`, `09-Reference/`, `10-Liminal/`, and `00-Creek-Meta/Skills/` (deployed skill tree) — treat this list as illustrative, not exhaustive, when doing a post-purge audit. The compliance audit log itself (`00-Creek-Meta/audit/purge.jsonl`) is JSONL, not Markdown, and is intentionally excluded — `affected_fragments` in audit entries keeps the real ID for forensic reconstruction (GAP-004).

   A wiki-link is matched **the way Obsidian resolves one**, not by string equality against the title (#903). `[[secret title]]`, `[[SECRET TITLE#Heading]]` and `[[Secret Title#h|display]]` all reach `Secret Title.md`, and all three used to survive the purge with the private title intact. The scrub now finds every link with one bracket regex, strips the `#heading` and `|alias` suffixes exactly as `LinkIndex.resolve` documents its caller doing, and compares `casefold()` to `casefold()` — the same relation the link index falls back to. Two names are scrubbed, not one: the declared `title` **and** the filename stem, because this vault's paths are title-derived and `01-Fragments/Journal/2026-03-11 therapy session.md` is itself the private string.

   Three names are scrubbed, not two: the declared `title`, the filename stem, **and** every entry in the page's own `aliases:` list (#1622). The name set is taken from `creek.vault.links.page_names` — the same function the surviving-claimant scan reads *other* pages with — because asking a narrower question of the purged page than of every other page in the vault is precisely what left `[[Codename Raven]]` standing after a purge of a fragment declaring that alias.

   **Markdown links are matched on their target, never on their display text** (#1622). `[text](Secret%20Title.md)` is a vault link — it is the shape `BrokenLinkScanner` has always counted in the link graph — and it survived the purge with the private title in the URL. The target is normalised the way a vault path resolves to a page name: any `#anchor` or `?query` suffix is dropped, a `<...>`-wrapped target is unwrapped, the URL is percent-decoded (Obsidian writes `Secret%20Title.md` whenever it generates the link itself), the folder prefix and a `.md` extension are stripped, and what remains goes through the *same* exact-before-folded decision the wiki-link pass uses. Removals are counted separately, as `markdown_links_removed`, on the CLI and in the audit `outcome` line; `wikilinks_removed` / `references_scrubbed` keep meaning wiki-links only.

   An **unencoded space inside the target is part of the path**, not the end of it. CommonMark ends a destination at whitespace only when a quoted `"title"` follows, so `[t](Secret Title.md "note")` links to `Secret Title.md` while `[read them](Secret Title Notes.md)` links to `Secret Title Notes.md` — a different page, which the purge leaves alone. Reading the first whitespace-delimited token unconditionally (as `BrokenLinkScanner` does, where a wrong guess only miscounts) would delete an operator's live link to a page this purge never touched.

   Two things are deliberately **out of scope**, and both are pinned by test. An external link whose display text merely collides with the title — `[Secret Title](https://example.com/unrelated)` — is left byte-identical: it points at something the purge never touched, and the string it would leave behind is prose either way. Bare prose mentions of the title are likewise untouched; this step scrubs links plus word-boundary fragment-ID mentions, and widening it to arbitrary title text would make it a redaction pass rather than a link scrub. `creek redact` is the tool for that.

   A **surviving** page's own `aliases:` entry is never edited (#1622). An `aliases` entry names the page it is written on, so deleting one destroys a live page's declared identity rather than removing a reference to the purged one. The *link* spelled that way is still removed — the exact-case shelter subtracts the purged page's own spellings, so a survivor claiming the purged name cannot switch the primary scrub off — but the frontmatter stays byte-identical.

   The fold stops at exactly one place, and stopping there is the point. `LinkIndex.resolve` tries its **exact-case** index before the folded one, so when two pages differ only in case — one being purged, one surviving — `[[ALPHA]]` names the survivor while `[[Alpha]]` names the target. A link spelled the way a *surviving* page spells itself is left alone. Over-matching on this path destroys an operator's live link to a page the purge never touched, which is as unacceptable as leaving the private title behind; both directions are pinned by test. The shelter never covers the purged page's **own** exact-case spelling: it exists for a case variant, and letting it cover the page's own name would mean any other `.md` claiming that string — a sibling fragment titled the same thing, an intimate-body stub, an Adepthood staging file this purge deletes moments later — switched the primary scrub off and left `[[Secret Title]]` standing.

   The rewrite is **byte-level** (#948). `read_text`/`write_text` could not scrub a file that is not valid UTF-8 at all — the reference simply survived, in a file the audit log reported as clean — and on a file that *is* valid it applied universal-newline translation, so removing one wiki-link from a CRLF note silently converted every line ending in it. Reading and writing bytes removes both: the substitution moves the bytes it was asked to move and no others. Only the captured link *target* is decoded, and a target that is not valid UTF-8 cannot equal a title, so no lossy decode is ever performed and no U+FFFD is ever written.
...
| `locks/content-mutations.lock` | keep | Empty cross-process coordination file shared by journal upsert/withdrawal and classify/link/compile/manual-review writes. Deleting a held advisory-lock path would let another process create and lock a different inode, splitting the load-to-write critical section. It contains no vault-derived content. |
| `locks/embeddings.lock` | keep | Empty cross-process coordination file for cache save, scrub, and erasure verification. Deleting a held advisory-lock path would let another process create and lock a different inode, splitting the critical section. It contains no vault-derived content. |
...
A `keep` or `exempt` row shelters **exactly one regular file, at exactly that path**. That is a constraint on the list itself, not just an observation about today's entries: a row added with a *directory* in mind would silently shelter nothing, because the match is path equality. Six edges follow, and four of them were real defects (#1484, #1485, #1562):
...
| `link-through-a-wiped-alias` — a symlink whose *final* target survives, reached through an intermediate link that lives inside a content folder | unlinked |
| `link-traversing-a-wiped-directory` — a symlink whose target *path* walks through a content-folder directory before climbing back out to a surviving target | unlinked |
| `link-named-in-another-case` — a symlink naming a content folder in a spelling the filesystem accepts but a string comparison does not, e.g. `../01-fragments/Journal` | unlinked |
...
- **"Does this purge break the link" is asked of the whole resolution chain, not of its final destination** (#1562). The first three rows are decided by following the link one hop at a time and asking, of *every* path the resolution walks through, whether the content wipe is about to destroy it. Asking only where the chain ends — `Path.resolve()`, which collapses every hop and every `..` — answered "survives" for all three while the apply run met a dangling link and unlinked it: preview `0`, apply `1`, the #1485 divergence again. An intermediate *alias* inside `01-Fragments/` is destroyed by the wipe even when the tree it points at is outside the vault; a target path that walks through `01-Fragments/Journal/sub/../../..` needs `sub` to exist to resolve at all; and a hop is compared to the ten content folders by **filesystem identity** (`Path.samefile`) rather than by string, because on a case-insensitive filesystem `01-fragments` and `01-Fragments` are one directory and only the second spelling matches as text. The walk is capped at 40 hops, and a link that exhausts the cap — a symlink loop — is reported as *not* broken: this predicate can only ever widen what the sweep destroys, so it fails open, never closed.
...
Two properties are load-bearing and worth stating outright. The removal uses **`rmdir`, never `rmtree`** — it refuses a directory that is not empty, so the worst a bug in this pass can do is leave a directory standing, never destroy content on a wrong turn. Every filesystem error the pass can hit — `PermissionError` on a read-only directory, `EBUSY` on a mount point, or a concurrent writer putting a file back into a directory just judged empty — is caught and turned into exactly that outcome, a standing directory and a WARNING, rather than escaping into `purge_vault`. A tidiness pass must never veto an erasure, and `creek purge vault` catches only `ValueError`, so an escape would surface as a raw traceback after the content erasure had already completed. And a **symlink is never descended**: `Path.is_dir()` follows a link, so a pass that asked that question first would walk *through* a link and remove a tree outside the vault entirely. A link counts as an occupant of the directory holding it, which keeps that directory standing.
...
Both surfaces that render `deleted_files` bound it, and both state the remainder rather than dropping it silently (#1454). The CLI caps its table at 20 rows and prints how many it hid. The MCP payload caps the list at 100 paths and carries `deleted_files_total` — the true count — alongside a `deleted_files_truncated` flag; those filenames are title-derived, so an uncapped list is megabytes of *content* pushed into the calling model's context on a large vault. The two caps are deliberately separate numbers: one bounds a scrollback, the other bounds a context window. Nothing else about the payload changed — every `PurgeResult` field still reaches the caller by model dump, which is the property #1246 introduced and a pinned test now protects.

Both vault walks the engine performs — the fragment census and the vault-wide reference scrub — refuse anything that links out of the vault (#1454). They were the module's last two unguarded walks, next to pointer-following paths (`_resolve_pointer_in_vault`, `_in_any_staging_root`, `_contained_voice_artifact`) that had been fail-closed for some time. A planted `01-Fragments/x.md -> /elsewhere/notes.md` was read for its declared `id`, that id was copied verbatim into the append-only `purge.jsonl`, and `creek purge classifications` rewrote the link's target in place — `write_text` follows a symlink. A planted `01-Fragments/ext -> /elsewhere/` was worse and inconsistent: `rglob` recurses through a directory link on Python 3.11/3.12 and, since gh-77609, does not on 3.13, so the same vault behaved differently per interpreter. Both walks now use `os.walk(followlinks=False)` plus the one shared containment predicate, `creek._containment.escaping_child`, and both fail closed — a link whose containment cannot be proven is refused, not admitted. The refusal is logged at `WARNING` naming the link *as walked* and never its resolved target, which would be the exfiltration oracle the guard exists to close. An alias that stays inside the vault is still followed: containment is about leaving the vault, not about being a link.
...
`fragment_count` on a thread or eddy is decremented by **splicing the one integer**, not by reserialising the header (#949). The previous load-and-dump round trip went through PyYAML, which drops comments, alphabetises keys, normalises quoting and strips trailing whitespace — so purging a single fragment quietly reformatted metadata on every thread and eddy it touched, and could not run at all on a file whose body is not valid UTF-8, leaving the count wrong there. The splice reads the header only, replaces the digits in place, and writes the file back with every other byte — invalid sequences included — untouched. One fallback remains and is logged when it fires: a header whose `fragment_count` is absent or is not a plain integer cannot be spliced, and is reserialised as before, which is also what wrote it that way.
