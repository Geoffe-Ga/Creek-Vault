...
4. **Never let a Liminal note bypass tier inheritance.** An Unnamed / Synchronicity / Compost note derived from a `personal` and an `intimate` fragment is saved with `--tier intimate`. Paradox is the exception, and not a safe one: `creek save --target paradox` forces `open` with the full body regardless of the tier passed, so intimate material must be routed to a different target entirely — see `paradox.SKILL.md` rule 5 and [Creek-Vault#1491](https://github.com/Geoffe-Ga/Creek-Vault/issues/1491).
5. **Never silently down-tier.** `creek save` and `creek.save` refuse outright when `--tier`/`tier` is omitted — there is no default and no inheritance. If the source content touched intimate material, the calling agent must determine that and pass `--tier intimate` (or `tier: "intimate"` over MCP) itself; the tool never infers or downgrades a tier on its own.
