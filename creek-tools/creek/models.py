...
class Authorship(StrEnum):
    """Authorship classification for a fragment.

    Indicates who authored the content:
    - ``self``: Written by the user (journal, essay, human chat turns)
    - ``ai``: Generated by an AI assistant (assistant chat turns)
    - ``other``: Written by someone else (other people's Discord messages)
    - ``collaborative``: Co-created by the user and another party
    """
